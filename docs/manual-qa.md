...
E-Hentai gallery pages share the gallery Posted date and taxonomy, but retain distinct page source URLs, original URLs, and duplicate identities
yande.re and Konachan detail pages accept the current Post.register_resp payload as well as legacy Post.register list data; related-post registrations do not replace the requested detail post
...
Twitter / X single status pages collect only that status's media instead of silently switching to the author's timeline; videos retain the highest-bitrate MP4 variant
