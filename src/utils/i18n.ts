...
pero desactivará la lupa y la capacidad de arrastrar y mover imágenes.`,
  ...
  eagleConfigPreviewSourceFieldsText: [
    'source page = visible Eagle link; original image identity and collected author URLs are retained when available; duplicates use source/original identity',
    '来源页面作为 Eagle 可见链接；原图身份和采集到的作者 URL 会在可用时保留；去重同时使用来源与原图身份',
    'source page = visible Eagle link; original image identity and collected author URLs are retained when available; duplicates use source/original identity',
    'pagina de origen = enlace visible de Eagle; la identidad original y URLs de autor se conservan cuando estan disponibles; duplicados usan ambas identidades',
  ],
