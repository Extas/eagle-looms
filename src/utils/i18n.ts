...
pero desactivará la lupa y la capacidad de arrastrar y mover imágenes.`,
  ...
  eagleBaseUrlTooltip: [
    ...
    '本机 Eagle Web API 端点。localhost/IP 地址可以省略 http://；通过 ?token=... 提供的 V2 token 会被保留。',
    'Local Eagle Web API endpoint. http:// may be omitted for localhost/IP addresses. A V2 token supplied as ?token=... is preserved.',
  ...
  eagleFolderPresetDate: [
    'Site / Import date',
    '站点 / 导入日期',
    'Site / Import date',
    'Sitio / Fecha de importacion',
  ],
  ...
  eagleFolderPathTooltip: [
    'Use / to separate Eagle folders. Tokens: {site}, {date}, {gallery}, {chapter}, {copyright}, {character}, {author}. {date} is the local date when import starts; the source publication date stays in source:published:* and optional item-name prefixes.',
    '用 / 分隔 Eagle 文件夹。支持变量：{site}、{date}、{gallery}、{chapter}、{copyright}、{character}、{author}。{date} 是开始导入时的本地日期；来源发布日期保留在 source:published:* 标签和可选的文件名前缀中。',
    'Use / to separate Eagle folders. Tokens: {site}, {date}, {gallery}, {chapter}, {copyright}, {character}, {author}. {date} is the local date when import starts; the source publication date stays in source:published:* and optional item-name prefixes.',
    'Use / para separar carpetas de Eagle. Variables: {site}, {date}, {gallery}, {chapter}, {copyright}, {character}, {author}. {date} es la fecha local al iniciar la importacion; la fecha de publicacion permanece en source:published:* y en el prefijo opcional del nombre.',
  ],
  eagleImportLimit: [
    'New items per import',
    '每次导入的新项目上限',
    'New items per import',
    'Nuevos elementos por importacion',
  ],
  eagleImportLimitTooltip: [
    'Maximum new Eagle items written from the assets collected for one import action. Duplicate, same-session, and preflight-failed items do not consume this write limit. Paged result sites may stop after collecting this many candidates, so a run can write fewer new items.',
    '一次导入操作从已采集资产中最多写入的 Eagle 新项目数。重复项、同会话重复项和预检查失败项不占用写入额度。分页结果站点可能在采集到同样数量的候选项后停止，因此实际写入的新项目可能更少。',
    'Maximum new Eagle items written from the assets collected for one import action. Duplicate, same-session, and preflight-failed items do not consume this write limit. Paged result sites may stop after collecting this many candidates, so a run can write fewer new items.',
    'Numero maximo de nuevos elementos escritos desde los recursos recopilados. Los duplicados y fallos de preflight no consumen el limite. Los sitios paginados pueden detener la recopilacion al alcanzar ese numero de candidatos.',
  ],
  eagleConfirmMode: [
    'Import confirmation',
    '导入确认策略',
  ...
  eagleConfirmModeTooltip: [
    'Auto confirms only larger batches. Always confirms any write. The Never option skips routine confirmation, but safety review still appears for preflight failures or import-limit omissions.',
    'Auto 只确认较大的批量导入。Always 只要会写入就确认。Never 跳过确认，但预检查失败或超过导入上限时仍会确认。',
    'Auto confirms only larger batches. Always confirms any write. The Never option skips routine confirmation, but safety review still appears for preflight failures or import-limit omissions.',
    'Auto confirma solo lotes grandes. Always confirma cualquier escritura. Never omite confirmacion salvo fallos de preflight o limite.',
  ],
  eagleConfirmModeAuto: [
    'Auto',
    '自动',
    'Auto',
    'Auto',
  ],
  eagleConfirmModeAlways: [
    'Always',
    '总是确认',
    'Always',
    'Siempre',
  ],
  eagleConfirmModeNever: [
    'Never (except warnings)',
    '不确认（警告除外）',
    'Never (except warnings)',
    'Nunca (salvo advertencias)',
  ],
  eagleConfirmThreshold: [
    'Auto confirmation threshold',
    '自动确认阈值',
    'Auto confirmation threshold',
    'Umbral de confirmacion auto',
  ],
  eagleConfirmThresholdTooltip: [
    'In Auto mode, show the confirmation dialog when will-write is greater than this number. When at least one item is writable, preflight failures and import-limit omissions still force safety review.',
    'Auto 模式下，will-write 大于该数值时弹出确认。只要至少有一个项目可写，预检查失败和超过导入上限仍会强制进入安全复核。',
    'In Auto mode, show the confirmation dialog when will-write is greater than this number. When at least one item is writable, preflight failures and import-limit omissions still force safety review.',
    'En modo Auto, muestra confirmacion solo cuando will-write supera este numero. Fallos y limite fuerzan confirmacion.',
  ],
  eagleMaxSourceTags: [
    'Additional source tag limit',
    '额外来源标签上限',
    'Additional source tag limit',
    'Limite de etiquetas adicionales',
  ],
  eagleMaxSourceTagsTooltip: [
    'Maximum additional source-site semantic tags copied to each Eagle item. The source publication date is retained separately. copyright:, character:, and author: tags are namespaced and prioritized; other reliable source tags follow. Set to 0 to copy only the publication date when available.',
    '每个 Eagle 项目最多复制的额外来源站点语义标签数量；来源发布日期会独立保留。copyright:、character:、author: 使用命名空间并优先导入，其他可靠来源标签随后导入；设为 0 时仅保留可用的发布日期。',
    'Maximum additional source-site semantic tags copied to each Eagle item. The source publication date is retained separately. copyright:, character:, and author: tags are namespaced and prioritized; other reliable source tags follow. Set to 0 to copy only the publication date when available.',
    'Numero maximo de etiquetas semanticas adicionales por elemento. La fecha de publicacion se conserva por separado. copyright:, character: y author: tienen prioridad; 0 conserva solo la fecha cuando esta disponible.',
  ],
  eagleSkipDuplicates: [
  ...
  eagleSkipDuplicatesTooltip: [
    'Before writing, query Eagle by source URL, original URL, and legacy stable keys. Same-session and same-plan repeats are always skipped to prevent accidental double writes.',
    '写入前按来源 URL、原图 URL 和旧版稳定 key 查询 Eagle。同一会话和同一计划内的重复项始终跳过，避免意外重复写入。',
    'Before writing, query Eagle by source URL, original URL, and legacy stable keys. Same-session and same-plan repeats are always skipped to prevent accidental double writes.',
    'Antes de escribir, consulta Eagle por URL de origen. Las repeticiones de la misma sesion o plan siempre se omiten para evitar escrituras dobles.',
  ],
  eagleNameDatePrefix: [
  ...
  eagleConfigTestOk: [
    'Connected to Eagle {version}, library {library}, at {url}',
    '已连接 Eagle {version}，资料库：{library}，地址：{url}',
    'Connected to Eagle {version}, library {library}, at {url}',
    'Conectado a Eagle {version}, biblioteca {library}, en {url}',
  ],
  eagleConfigUnknownLibrary: [
    'unknown',
    '未知',
    'unknown',
    'desconocida',
  ...
  eagleConfigUnknownFolderTokens: [
    'Unknown tokens: {tokens}. Import is blocked until the folder rule is corrected.',
    '未知变量：{tokens}。修正文件夹规则之前不会执行导入。',
    'Unknown tokens: {tokens}. Import is blocked until the folder rule is corrected.',
    'Variables desconocidas: {tokens}. La importacion se bloquea hasta corregir la regla.',
  ],
  eagleConfigMalformedFolderTokens: [
    'Folder tokens contain unmatched or nested braces. Import is blocked until corrected.',
    '文件夹变量包含未配对或嵌套的大括号。修正之前不会执行导入。',
    'Folder tokens contain unmatched or nested braces. Import is blocked until corrected.',
    'Las variables contienen llaves sin pareja o anidadas. La importacion se bloquea hasta corregirlas.',
  ],
  ...
  eagleImportUnknownFolderRule: [
    'Eagle folder rule contains unknown tokens: {tokens}. Correct Eagle Folder Path in Config before importing.',
    'Eagle 文件夹规则包含未知变量：{tokens}。请先在配置中修正 Eagle 文件夹路径。',
    'Eagle folder rule contains unknown tokens: {tokens}. Correct Eagle Folder Path in Config before importing.',
    'La regla de carpeta contiene variables desconocidas: {tokens}. Corrige la ruta Eagle antes de importar.',
  ],
  ...
  eagleConfigPreviewSourceFieldsText: [
    'source page = Eagle link; original URL is submitted but V2 may not preserve it; collected author URLs are retained when available; duplicates use source URL + stable media identity',
    '来源页面 = Eagle 链接；原图 URL 会提交但 V2 可能不保留；采集到的作者 URL 会在可用时保留；去重使用来源 URL + 稳定媒体身份',
    'source page = Eagle link; original URL is submitted but V2 may not preserve it; collected author URLs are retained when available; duplicates use source URL + stable media identity',
    'pagina de origen = enlace Eagle; la URL original se envia pero V2 puede no conservarla; URLs de autor recopiladas se conservan cuando existen; duplicados usan URL de origen + identidad estable',
  ],
  ...
  eagleConfigPreviewBatchText: [
    'new writes: max {count} after preflight; paged candidates: may stop at {count}; duplicates: {duplicates}',
    '预检查后新写入：最多 {count}；分页候选：可能在 {count} 个时停止；重复项：{duplicates}',
    'new writes: max {count} after preflight; paged candidates: may stop at {count}; duplicates: {duplicates}',
    'nuevas escrituras: maximo {count} tras preflight; candidatos paginados: pueden detenerse en {count}; duplicados: {duplicates}',
  ],
  eagleConfigPreviewConfirm: [
    'Confirmation',
    '确认',
    'Confirmation',
    'Confirmacion',
  ],
  eagleConfigPreviewConfirmAuto: [
    'auto; confirm when will-write > {count}; failures and over-limit still confirm',
    '自动；will-write > {count} 时确认；失败和超过上限仍确认',
    'auto; confirm when will-write > {count}; failures and over-limit still confirm',
    'auto; confirma cuando will-write > {count}; fallos y limite confirman',
  ],
  eagleConfigPreviewConfirmAlways: [
    'always confirm when new items will be written',
    '只要会写入新项目就确认',
    'always confirm when new items will be written',
    'siempre confirma cuando se escriben elementos nuevos',
  ],
  eagleConfigPreviewConfirmNever: [
    'never confirm unless failures or over-limit need review',
    '不确认，除非失败或超过上限需要复核',
    'never confirm unless failures or over-limit need review',
    'nunca confirma salvo fallos o limite',
  ],
  ...
  eagleConfigPreviewAddDuplicates: [
    'existing Eagle items allowed; session repeats skipped',
    '允许与 Eagle 现有项目重复；仍跳过本会话重复项',
    'existing Eagle items allowed; session repeats skipped',
    'se permiten existentes en Eagle; se omiten repeticiones de sesion',
  ],
  ...
  eagleConfigPreviewTags: [
    'source publication date always; copyright:/character:/author: first, plus other source tags; max {count} others',
    '始终保留来源发布日期；其余标签优先 copyright:/character:/author:，最多 {count} 个',
    'source publication date always; copyright:/character:/author: first, plus other source tags; max {count} others',
    'fecha de publicacion siempre; copyright:/character:/author: primero; max {count} etiquetas adicionales',
  ],
  eagleConfigPreviewNoTags: [
    'source publication date only, when available',
    '仅保留来源发布日期（如果可用）',
    'source publication date only, when available',
    'solo fecha de publicacion, cuando este disponible',
  ],
  eagleSummaryTitle: [
    'Eagle import',
    'Eagle 导入',
  ...
  eagleToastImported: [
    'Imported {count} {item} to Eagle',
    '已导入 {count} 张图片到 Eagle',
    'Imported {count} {item} to Eagle',
    'Importadas {count} {item} a Eagle',
  ],
  eagleToastImage: [
    'image',
    '图片',
  ...
  eagleToastNoNewItems: [
    'No new Eagle items',
    '没有新的 Eagle 项目',
    'No new Eagle items',
    'Sin elementos nuevos de Eagle',
  ],
  eagleToastSkippedSuffix: [
    ', skipped {count}',
    '，跳过 {count}',
    ', skipped {count}',
    ', omitidos {count}',
  ...
  eagleToastFailedSuffix: [
    ...
    '，失败 {count}。查看导入结果',
    ', failed {count}. See import result',
    ', fallidos {count}. Ver resultado',
  ],
  eagleToastFailedOnly: [
    'Failed {count}. See import result.',
    '失败 {count}。查看导入结果。',
    'Failed {count}. See import result.',
    'Fallidos {count}. Ver resultado.',
  ...
  eaglePlanSkippedBeforeWriting: [
    'skipped before writing {count}{reasons}',
    '写入前跳过 {count}{reasons}',
    'skipped before writing {count}{reasons}',
    'omitidos antes de escribir {count}{reasons}',
  ],
  ...
  eaglePlanVisibleTagsMax: [
    'additional source tags max {count}',
    '额外来源标签最多 {count}',
    'additional source tags max {count}',
    'etiquetas de origen adicionales max {count}',
  ],
  eaglePlanDuplicates: [
    'duplicates {policy}',
    '重复项 {policy}',
  ...
  eaglePlanDuplicatesAllowed: [
    ...
    '允许与 Eagle 现有项目重复；仍跳过本会话重复项',
    'existing Eagle items allowed; session repeats skipped',
    'existentes en Eagle permitidos; repeticiones de sesion omitidas',
  ],
  pixivUgoiraMode: [
    "Pixiv Ugoira Mode",
    "Pixiv 动图模式",
...
<br>  Alphabetically: Detecta si los nombres de archivo originales son consistentes con el orden de lectura bajo el orden alfabético (Linux). Si son consistentes, conserva los nombres de archivo originales; de lo contrario, antepone un número a los nombres originales para garantizar el orden correcto. `,
  ...
  eagleImportCheckingEagle: [
    'Checking Eagle...',
    '正在检查 Eagle...',
    'Checking Eagle...',
    'Comprobando Eagle...',
  ],
  eagleImportWritingToEagle: [
    'Writing to Eagle',
    '正在写入 Eagle',
    'Writing to Eagle',
