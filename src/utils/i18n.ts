...
pero desactivará la lupa y la capacidad de arrastrar y mover imágenes.`,
  ...
  eagleImportLimitTooltip: [
    'Maximum ready assets written by one Eagle import action, including multi-file current items. Paged result sites may also stop collection at this limit.',
    '一次 Eagle 导入操作最多写入的已就绪资产数量，包括会展开为多个文件的当前项目。分页结果站点也会按这个上限停止采集。',
    'Maximum ready assets written by one Eagle import action, including multi-file current items. Paged result sites may also stop collection at this limit.',
    'Numero maximo de assets listos escritos por una accion de importacion de Eagle, incluidos elementos actuales con varios archivos.',
  ],
  eagleConfirmMode: [
    'Import confirmation',
    '导入确认策略',
  ...
  eagleConfirmModeTooltip: [
    'Auto confirms only larger batches. Always confirms any write. Never skips confirmation unless the plan has preflight failures or import-limit omissions.',
  ...
  eagleConfirmThresholdTooltip: [
    'In Auto mode, show the confirmation dialog only when will-write is greater than this number. Failures and import-limit omissions still force confirmation.',
    'Auto 模式下，只有 will-write 大于该数值时才弹确认。预检查失败和超过导入上限仍会强制确认。',
    'In Auto mode, show the confirmation dialog only when will-write is greater than this number. Failures and import-limit omissions still force confirmation.',
